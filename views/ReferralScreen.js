...
const ReferralScreen = () => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      Alert.alert(t('referralScreen.errorTitle'), t('referralScreen.loadError'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleShare = async () => {
    ...
    const message = t('referralScreen.shareMessage', { url: info.shareUrl });
    try {
      await Share.share({ message, url: info.shareUrl, title: t('referralScreen.shareTitle') });
    } catch (e) {
      Alert.alert(t('referralScreen.shareErrorTitle'), e?.message || t('referralScreen.shareError'));
    }
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: colors.surface }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>{t('referralScreen.title')}</Text>
        <View style={{ width: 40 }} />
      </View>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          {!!prefillCode && !user?.referredBy && (
            <View style={[styles.rewardBanner, { backgroundColor: 'rgba(0,194,203,0.15)' }]}>
              <Text style={[styles.rewardText, { color: colors.textPrimary, marginBottom: 12 }]}>
                {t('referralScreen.confirmCode', { code: prefillCode })}
              </Text>
              <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
                ...
                >
                  {prefillSubmitting ? (
                    ...
                    <Text style={styles.shareBtnText}>{t('referralScreen.validate')}</Text>
                  )}
                </TouchableOpacity>
              </View>
            </View>
          ...
          {info?.rewardActive && (
            <View style={[styles.rewardBanner, { backgroundColor: 'rgba(0,194,203,0.15)' }]}>
              <Text style={[styles.rewardText, { color: colors.textPrimary }]}>
                {t('referralScreen.rewardActive')}
                {info.rewardExpiresAt ? t('referralScreen.rewardActiveUntil', { date: new Date(info.rewardExpiresAt).toLocaleDateString() }) : ''}.
              </Text>
            </View>
          )}

          <View style={[styles.codeCard, { backgroundColor: colors.surface }]}>
            <Text style={[styles.codeLabel, { color: colors.textPrimary, opacity: 0.5 }]}>{t('referralScreen.yourCode')}</Text>
            <Text style={[styles.code, { color: colors.textPrimary }]}>{info?.referralCode || '—'}</Text>

            <TouchableOpacity style={styles.shareBtn} onPress={handleShare} accessibilityLabel={t('referralScreen.shareLabel')}>
              <Text style={styles.shareBtnText}>{t('referralScreen.shareButton')}</Text>
            </TouchableOpacity>
          ...
          <View style={[styles.progressCard, { backgroundColor: colors.surface }]}>
            <Text style={[styles.progressLabel, { color: colors.textPrimary }]}>
              {t('referralScreen.progressLabel', { count: info?.currentMonthValidatedCount ?? 0, target: info?.targetCount ?? 5 })}
            </Text>
          ...
          <Text style={[styles.sectionTitle, { color: colors.textPrimary, opacity: 0.5 }]}>{t('referralScreen.invitedFriends')}</Text>
          {history.length === 0 ? (
            <Text style={[styles.emptyText, { color: colors.textPrimary, opacity: 0.5 }]}>
              {t('referralScreen.noInvites')}
            </Text>
          ) : (
            history.map((entry, index) => (
              <View key={index} style={[styles.historyRow, { backgroundColor: colors.surface }]}>
                <Text style={[styles.historyName, { color: colors.textPrimary }]}>
                  {entry.referredUser?.username || t('referralScreen.defaultUser')}
                </Text>
                <Text
                  style={[
                ...
                >
                  {entry.status === 'validated' ? t('referralScreen.statusValidated') : entry.status === 'void' ? t('referralScreen.statusVoid') : t('referralScreen.statusPending')}
                </Text>
              </View>
            ))
          )}
