    ...
    const bootstrap = async () => {
      try {
        ...
        const match = items.find(
          (c) => (c.otherUser?._id || c.otherUser?.id) === (conversationUser._id || conversationUser.id),
        );
        ...
        if (convId) {
          ...
          if (last?.id || last?._id) {
            try {
              await markConversationRead(convId, { messageId: last.id || last._id });
            } catch (_) {}
          }
        } else {
          if (mounted) {
      ...
      try {
        if (asset.type === 'image') {
          ...
          const uploaded = await uploadChatMedia({
            media: { uri: optimized.uri, name: `img_${Date.now()}.jpg`, type: 'image/jpeg' },
          });
          ...
          setMessages((prev) => [
            ...prev,
            { id: `tmp_${Date.now()}`, type: 'image', mediaUrl, timestamp: new Date().toISOString(), senderId: 'me' },
          ]);
        } else if (asset.type === 'video') {
          const thumbUri = await generateVideoThumbnail(asset.uri);
          const uploaded = await uploadChatMedia({
            media: { uri: asset.uri, name: `vid_${Date.now()}.mp4`, type: 'video/mp4' },
            thumbnail: thumbUri ? { uri: thumbUri, name: `thumb_${Date.now()}.jpg`, type: 'image/jpeg' } : undefined,
          });
          ...
          await sendChatMessage({
            targetUserId: conversationUser.id || conversationUser._id,
            type: 'video',
            mediaUrl,
            thumbnailUrl,
          });
    ...
    >
      ...
      >
        <TouchableOpacity
          ...
          style={[styles.backButton, { backgroundColor: colors.accent + '15' }]}
        >
          ...
          <View style={styles.headerAvatarContainer}>
            {conversationUser.photo ? (
              ...
              <View style={[styles.headerAvatarPlaceholder, { backgroundColor: colors.accent + '20' }]}>
                <Text style={[styles.headerAvatarInitial, { color: colors.accent }]}>
                  {conversationUser.username?.[0]?.toUpperCase() ?? '?'}
                </Text>
              </View>
            )}
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate('UserProfile', { userId: conversationUser.id || conversationUser._id })}
          style={[styles.backButton, { backgroundColor: colors.accent + '15' }]}
        >
          <Text style={{ fontSize: 20, color: colors.accent }}>👤</Text>
        </TouchableOpacity>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <MessageBubble
            ...
            showReadReceipt={
              (item.id || item._id) === (messages[messages.length - 1]?.id || messages[messages.length - 1]?._id)
            }
          />
        )}
      ...
      <View style={[styles.inputWrapper, { paddingBottom: Platform.OS === 'ios' ? 30 : 15 }]}>
        <View
          style={[
            ...
            { backgroundColor: colors.surface, shadowColor: isDark ? '#000' : colors.accent },
          ]}
        >
          <TouchableOpacity
            onPress={handlePickMedia}
            style={[styles.attachButton, { backgroundColor: colors.accent + '15' }]}
          >
