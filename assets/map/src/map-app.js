...
(function () {
  ...
  // Décale les contrôles bas de MapLibre (attribution © MapTiler / © OSM) pour
  // qu'ils ne passent pas sous la barre de nav flottante + VibeFAB côté RN.
  function applyBottomInset(px) {
    var n = Math.max(0, parseInt(px, 10) || 0);
    var el = document.getElementById('__mapBottomInset');
    if (!el) {
      el = document.createElement('style');
      el.id = '__mapBottomInset';
      document.head.appendChild(el);
    }
    el.textContent =
      '.maplibregl-ctrl-bottom-left,.maplibregl-ctrl-bottom-right{bottom:' + n + 'px !important;}';
  }
  ...
  function applyRender(payload) {
    ...
    var bottomInset = payload.bottomInset || 0;
    var hasUserLocation = !!payload.hasUserLocation;

    lastLocations = locations;
    lastPalette = palette;
    applyBottomInset(bottomInset);

    // Au tout premier rendu : cadrer sur l'utilisateur + les lieux proches
    // plutôt que de rester sur un zoom fixe qui peut laisser l'utilisateur
    // hors champ.
    function initialFit() {
      if (didInitialFit) return;
      var pts = [];
      if (hasUserLocation && Array.isArray(center) && center.length >= 2) pts.push(center);
      (locations || []).forEach(function (l) {
        if (l && Array.isArray(l.coords) && l.coords.length >= 2) pts.push(l.coords);
      });
      if (pts.length < 2) return; // rien à cadrer : on garde center/zoom
      didInitialFit = true;
      var b = pts.reduce(function (acc, p) {
        return acc.extend(p);
      }, new maplibregl.LngLatBounds(pts[0], pts[0]));
      map.fitBounds(b, {
        padding: { top: 64, left: 40, right: 40, bottom: bottomInset + 48 },
        maxZoom: 15,
        duration: 0,
      });
    }
    ...
    // Si le 1er payload n'avait pas encore de lieux, cadrer dès qu'ils arrivent.
    if (ready) initialFit();
  }
