...
const MessageBubble = ({ message, isMe, recipientAvatar, showReadReceipt }) => {
  ...
  const renderContent = () => {
    switch (message.type) {
      case 'image':
        return (
          <Image source={{ uri: proxifyImageUrl(message.mediaUrl) }} style={styles.mediaContent} resizeMode="cover" />
        );
      case 'video':
        return (
      ...
      default:
        return <Text style={[styles.text, { color: isMe ? '#fff' : colors.textPrimary }]}>{message.content}</Text>;
    }
  };
